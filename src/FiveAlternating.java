...
public class FiveAlternating extends MyBot implements Strategy {
    ...
    private int findLowestCardInRange(int start, int end) {
        return myCards.stream()
                ...
                .min(Integer::compare)
                .orElse(-1); // Returns -1 if no card is found
    }
    ...
    private int findMiddleCardInRange(int start, int end) {
        ...
        if (!cardsInRange.isEmpty()) {
            return cardsInRange.get(cardsInRange.size() / 2); // Return the middle card
        }
        return -1; // Return -1 if no card is found
    }
    ...
    private int findRandomCardInRange(int start, int end) {
        ...
        if (!cardsInRange.isEmpty()) {
            ...
            return cardsInRange.get(randomIndex); // Return a random card from the range
        }
        return -1; // Return -1 if no card is found
    }
